...
@onready var cs_path_button: Button = grid.get_node("CSPathButton")
@onready var fs_proj_path_edit: LineEdit = grid.get_node("FSProjPathEdit")
@onready var fs_proj_path_button: Button = grid.get_node("FSProjPathButton")
...
func init(p_plugin: EditorPlugin) -> void:
	...
	cs_path_button.connect("pressed", file_dialog_cs.popup_centered_ratio.bind(0.75))
	# warning-ignore:return_value_discarded
	fs_proj_path_button.connect("pressed", file_dialog_fs.popup_centered_ratio.bind(0.75))
	# warning-ignore:return_value_discarded
...
func _reset_final_path(_p_text = null) -> void:
	final_path.text = fs_proj_path_edit.text.get_base_dir() + "/" + (cs_path_edit.text.get_file().get_basename() + "Fs.fs")
