...
@onready var path_button: Button = grid.get_node("PathButton")
...
func init(p_plugin: EditorPlugin) -> void:
	...
	path_button.connect("pressed", file_dialog.popup_centered_ratio.bind(0.75))
	# warning-ignore:return_value_discarded
...
func _reset_final_path(_p_text) -> void:
	final_path.text = path_edit.text + "/" + ((name_edit.text + "/" if create_dir_check.pressed else "") + name_edit.text + ".fsproj")
